    ...
    public class FileStream : Stream
    {
        ...
        public FileStream(string path, FileMode fileMode, FileAccess fileAccess, FileInfo fileInfo)
        {
            ...
            if (fileInfo.Exists())
            {
                try
                {
                    _data = Window.Atob(LocalStorage.GetItem(_name));
        ...
        private void Consolidate()
        {
            ...
            if (_data.Length < _newDataPosition)
            {
                ...
                while (_data.Length + filler.Length < _newDataPosition)
                    filler.Append('\0');
                _data += filler.ToString();
            }
            long p2 = _newDataPosition + _newData.Length;
        ...
        public override void Flush()
        {
            if (!_isDirty)
                return;
            Consolidate();
            LocalStorage.SetItem(_name, Window.Btoa(_data));
            _isDirty = false;
        }
        ...
        public override void WriteByte(int b)
        {
            ...
            else if (_newDataPosition + _newData.Length != _position)
            {
                Consolidate();
                _newDataPosition = _position;
                _newData = new StringBuilder();
